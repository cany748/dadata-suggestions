...
expect.extend({
  ...
  toHaveText(received, expected) {
    ...
    return {
      pass,
      message: () =>
        pass ? `Expected element not to have text "${expected}"` : `Expected element to have text "${expected}", but got "${text}"`,
    };
  },

  toBeVisible(received) {
    const element = received instanceof $ ? received : $(received);
    const pass = element.is(":visible");
  ...
  toBeHidden(received) {
    const element = received instanceof $ ? received : $(received);
    const pass = element.is(":hidden") || received.offsetParent === null;

    return {
      pass,
      message: () => (pass ? `Expected element not to be hidden` : `Expected element to be hidden`),
    };
  },

  toHaveClass(received, expected) {
    const element = received instanceof $ ? received : $(received);
    const pass = element.hasClass(expected);

    return {
      pass,
      message: () => (pass ? `Expected element not to have class "${expected}"` : `Expected element to have class "${expected}"`),
    };
  },

  toHaveAttr(received, attr, value) {
    const element = received instanceof $ ? received : $(received);
    const actualValue = element.attr(attr);
    const pass = value === undefined ? actualValue !== undefined : actualValue === value;
