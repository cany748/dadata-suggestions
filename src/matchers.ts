...
function sameParentChecker(preprocessFn: (val: any) => any) {
  return function (suggestions: any[]) {
    ...
    const aliens = suggestions.filter(function (suggestion: any) {
 ...
 * @type {Function}
 */
const haveSameParent = sameParentChecker(function (val: any) {
  return val;
});
...
function minusWithPartialMatching(array1: string[], array2: string[]) {
  ...
  return array1.filter(function (el: string) {
    return !array2.some(function (el2: string) {
